...
const getChanges = async (localpath, isDirectory, remoteBranchHash) => {
  ...
  try {
    ...
    if (error.message.trim().slice(0, 31) === 'fatal: Not a valid object name ') {
      console.error(
        `The remote branch copy for path ${localpath} is not found in local storage. Maybe the remote branch is newer and not synced yet or files were renamed/moved locally.`
          .red.bold,
      );
    } else {
  ...
  try {
    ...
    const stats = (
      await Promise.allSettled(
        remoteFilesHash.map((fh) => fs.stat(isDirectory ? path.join(localpath, fh.file) : path.resolve(path.dirname(localpath), fh.file))),
      )
  ...
  localFiles = isDirectory ? readdirRecursive(localpath).map((f) => path.relative(localpath, f)) : [path.parse(localpath).base];

  localFilesHash = (await spawnChild('git', ['hash-object', '--stdin-path'], localFiles.map((f) => path.join(localpath, f)).join('\n')))
    .split('\n')
...
const buildFieldInfo = (field, original) => {
  const source =
    original != null
      ? `(${original.relation.schema != null && original.relation.schema != 'public' ? `${original.relation.schema}.` : ''}${original.relation.name}.${
          original.field
        })`
      : '';

  return (
    `${field.kind} "${field.names.local || field.names.original}" ${
      field.type.schema != null && field.type.schema === 'pg_catalog' ? '' : `${field.type.schema}.`
    }${field.type.name.toUpperCase()} ${field.type.mods.length > 0 ? `(${field.type.mods.join(',')})` : ''}` +
    ...
    if (item.old.error == null && item.new.error == null) {
      ...
      item.new.fields.forEach((newVersion, fid) => {
        ...
        if (old == null) {
          ...
          if (
            (item.old.fieldsOriginal[fid] != null &&
              item.new.fieldsOriginal[fid] != null &&
...
const check = async (params) => {
  ...
  if (ddlFiles.deleted.length > 0 || ddlFiles.changed.length > 0 || ddlFiles.new.length > 0) {
    const ddlsOld = (await Promise.allSettled(ddlFiles.remote.map((fh) => spawnChild('git', ['cat-file', 'blob', fh.hash])))).map((p) =>
      p.status === 'fulfilled' ? p.value.toString() : null,
    );
    ...
    const dmlsOld = (await Promise.allSettled(dmlFiles.remote.map((fh) => spawnChild('git', ['cat-file', 'blob', fh.hash])))).map((p) =>
      p.status === 'fulfilled' ? p.value.toString() : null,
    );
    ...
    const ddlsNew = (
      await Promise.allSettled(ddlFiles.remote.map((fh) => fs.readFile(ddlPathIsDirectory ? path.resolve(params.ddl, fh.file) : path.resolve(params.ddl))))
    ...
    const dmlsNew = (
      await Promise.allSettled(dmlFiles.remote.map((fh) => fs.readFile(dmlPathIsDirectory ? path.resolve(params.dml, fh.file) : path.resolve(params.dml))))
    ).map((p) => (p.status === 'fulfilled' ? p.value.toString() : null));
    ...
    if (dmlFiles.changed.length > 0) {
      ...
      console.log(
        `Need to refresh types for ${dmlFiles.changed.length.toString().yellow.bold} of ${dmlFiles.remote.length.toString().green.bold}` +
          ` DMLs`.bold +
          ` with ${ddlFiles.deleted.length > 0 || ddlFiles.changed.length > 0 ? 'CHANGED'.green.bold : 'UNCHANGED'.green.bold}` +
          ` DDL`.bold,
      );
      ...
      const dmls = (
        await Promise.allSettled(
          dmlFiles.changed.flatMap((fh) => [
            spawnChild('git', ['cat-file', 'blob', fh.hash]),
            fs.readFile(dmlPathIsDirectory ? path.resolve(params.dml, fh.file) : path.resolve(params.dml)),
          ]),
        )
      ).map((p) => (p.status === 'fulfilled' ? p.value.toString() : null));
